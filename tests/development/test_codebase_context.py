...
# ---------------------------------------------------------------
print("\n=== Test 15: Sandbox fixes sync to generated_code ===")
with tempfile.TemporaryDirectory() as tmpdir:
    root = Path(tmpdir)
    gen_dir = root / "generated_code"
    sandbox_dir = root / "sandbox" / "src"
    gen_dir.mkdir()
    sandbox_dir.mkdir(parents=True)

    (gen_dir / "comm.c").write_text("stale generated version\n")
    (sandbox_dir / "comm.c").write_text("fixed sandbox version\n")

    synced = _sync_generated_from_sandbox(
        gen_dir,
        {"comm.c": sandbox_dir / "comm.c"},
    )
    check("sync reports copied file", synced == 1, f"got {synced}")
    check(
        "generated_code receives sandbox fix",
        (gen_dir / "comm.c").read_text() == "fixed sandbox version\n",
